...
EOF
    ...
    local added_count=$(comm -13 "$before_file" "$after_file" | wc -l)
...
EOF
    ...
    if [[ $added_count -gt 0 ]]; then
        comm -13 "$before_file" "$after_file" | while read -r pkg; do
...
EOF
    ...
    if [[ $removed_count -gt 0 ]]; then
        comm -23 "$before_file" "$after_file" | while read -r pkg; do
            echo "  - $pkg" >> "$report_file"
        done
    else
...
validate_config() {
    ...
    # 检查是否有语法错误
    if grep -q "^CONFIG_.*=y$" "$config_file"; then
        log_info "配置文件格式正确"
    else
        log_warn "配置文件可能存在问题"
    fi
